...
in
{
  programs.ncmpcpp = {
    ...
    config = {
      ...
      alternative_header_second_line_format = "{{$3$b%a$/b$9}{ - $8$b%b$/b$9}{ $0(%y)$9}}|{%D}";
    ...
    bindings = {
      ...
      ctrl-s = [ "sort_playlist" "toggle_browser_sort_mode" "toggle_media_library_sort_mode" ];
      ctrl-v = [ "select_range" ];
      d = [ "delete_playlist_items" "delete_browser_items" "delete_stored_playlist" ];
      e = [ "edit_song" "edit_library_tag" "edit_library_album" "edit_directory_name" "edit_playlist_name" "edit_lyrics" ];
      enter = [ "enter_directory" "toggle_output" "run_action" "play_item" "slave_screen" ];
      f1 = [ "show_help" ];
      g = [ "move_home" ];
      h = [ "previous_column" "master_screen" "volume_down" "jump_to_parent_directory" ];
      i = [ "show_song_info" ];
      insert = [ "select_item" ];
      j = [ "scroll_down" ];
      k = [ "scroll_up" ];
      l = [ "enter_directory" "run_action" "play_item" "next_column" "slave_screen" "volume_up" ];
      ...
      space = [ "add_item_to_playlist" "toggle_lyrics_update_on_song_change" "toggle_visualization_type" ];
      tab = [ "next_screen" ];
      u = [ "update_database" ];
      v = [ "reverse_selection" ];
      w = [ "toggle_find_mode" ];
